  ...
  const handleStatusChange = async (appointment) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const employee = employees.find(emp => emp.emp_no === appointment.emp_no);
        if (employee) {
          navigate("../newvisit", { state: { data: employee, reference: false } });
        } else {
          console.warn("Employee data not found for emp_no:", appointment.emp_no, "after status update.");
          navigate("../newvisit", { state: { search: appointment.emp_no, reference: false } });
        }
      } else {
              ...
              // Use the refined message function here
                 ...
                 filteredAppointments.map((appointment) => (
                    <tr key={appointment.id} className="border-b border-gray-100 hover:bg-gray-50 transition group">
                      ...
                      <td className="px-3 py-2 text-xs text-gray-700 text-left truncate">{appointment.aadhar_no || '-'}</td>
