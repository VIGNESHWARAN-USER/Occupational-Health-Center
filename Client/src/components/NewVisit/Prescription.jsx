...
const Prescription = ({ data, onPrescriptionUpdate, condition, register, mrdNo }) => {
  ...
  const medicineForms = {
    tablets: "Tablet", injections: "Injection", syrups: "Syrup",
    drops: "Drops", creams: "Creams", respules: "Respules",
    lotions: "Lotions", fluids: "Fluids", powder: "Powder",
  ...
  const getDefaultRow = useCallback(
    () => ({
      chemicalName: "", brandName: "", doseVolume: "", qty: "",
      timing: [], food: "", days: "", serving: "",
      issuedIn: "", issuedOut: "", prescriptionOut: "", expiryDate: "",
    }),
    []
  );
  ...
  useEffect(() => {
    const initializePrescription = () => {
      const initialData = {
        tablets: [getDefaultRow()], injections: [getDefaultRow()],
        syrups: [getDefaultRow()], drops: [getDefaultRow()],
        creams: [getDefaultRow()], respules: [getDefaultRow()],
        lotions: [getDefaultRow()], fluids: [getDefaultRow()],
        powder: [getDefaultRow()], sutureProcedureItems: [getDefaultRow()],
        dressingItems: [getDefaultRow()], others: [getDefaultRow()],
      ...
      if (condition && existingPrescription) {
        ...
        const createItem = (item) => ({
          chemicalName: item.chemicalName || "", brandName: item.brandName || "",
          doseVolume: item.doseVolume || "", serving: item.serving || "",
          qty: item.qty || "", timing: mapTiming(item.timing),
          food: item.food || "", days: item.days || "",
          expiryDate: item.expiryDate || "", issuedIn: item.issuedIn || "",
          issuedOut: item.issuedOut || "", prescriptionOut: item.prescriptionOut || "",
        });

        initialData.tablets = existingPrescription.tablets?.length > 0 ? existingPrescription.tablets.map(createItem) : [getDefaultRow()];
        initialData.injections = existingPrescription.injections?.length > 0 ? existingPrescription.injections.map(createItem) : [getDefaultRow()];
        initialData.syrups = existingPrescription.syrups?.length > 0 ? existingPrescription.syrups.map(createItem) : [getDefaultRow()];
        initialData.drops = existingPrescription.drops?.length > 0 ? existingPrescription.drops.map(createItem) : [getDefaultRow()];
        initialData.creams = existingPrescription.creams?.length > 0 ? existingPrescription.creams.map(createItem) : [getDefaultRow()];
        initialData.respules = existingPrescription.respules?.length > 0 ? existingPrescription.respules.map(createItem) : [getDefaultRow()];
        initialData.lotions = existingPrescription.lotions?.length > 0 ? existingPrescription.lotions.map(createItem) : [getDefaultRow()];
        initialData.fluids = existingPrescription.fluids?.length > 0 ? existingPrescription.fluids.map(createItem) : [getDefaultRow()];
        initialData.powder = existingPrescription.powder?.length > 0 ? existingPrescription.powder.map(createItem) : [getDefaultRow()];
        initialData.sutureProcedureItems = existingPrescription.suture_procedure?.length > 0 ? existingPrescription.suture_procedure.map(createItem) : [getDefaultRow("sutureProcedureItems")];
        initialData.dressingItems = existingPrescription.dressing?.length > 0 ? existingPrescription.dressing.map(createItem) : [getDefaultRow("dressingItems")];
        initialData.others = existingPrescription.others?.length > 0 ? existingPrescription.others.map(createItem) : [getDefaultRow()];
        initialData.nurse_notes = existingPrescription.nurse_notes || "";
        initialData.consulted_doctor = existingPrescription.consulted_doctor || "";
      }

      setTablets(initialData.tablets); setInjections(initialData.injections);
      setSyrups(initialData.syrups); setDrops(initialData.drops);
      setCreams(initialData.creams); setRespules(initialData.respules);
      setLotions(initialData.lotions); setFluids(initialData.fluids);
      setPowder(initialData.powder); setSutureProcedureItems(initialData.sutureProcedureItems);
      setDressingItems(initialData.dressingItems); setOthers(initialData.others);
      setNurseNotes(initialData.nurse_notes); setConsultedDoctor(initialData.consulted_doctor);
    };
  ...
  const addRow = (type) => {
    ...
    const typeMap = {
        tablets: setTablets, injections: setInjections, syrups: setSyrups,
        drops: setDrops, creams: setCreams, respules: setRespules,
        lotions: setLotions, fluids: setFluids, powder: setPowder,
        sutureProcedureItems: setSutureProcedureItems, dressingItems: setDressingItems, others: setOthers,
    };
    if (typeMap[type]) {
  ...
  const fetchDoseSuggestions = useCallback(
    debounce(async (brandName, chemicalName, medicineForm, type, index) => {
      ...
      try {
        ...
        } else { setShowDoseSuggestions((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));}
      } catch (error) { /* ... handle error ... */ }
    }, 500), [doseManuallyEntered]
  );
  ...
  const handleInputChange = (e, type, index, field) => {
    ...
    if (type === "consultedDoctor") {
        ...
        if (value.length >= 2) { fetchDoctorSuggestions(value); }
        else { setShowDoctorSuggestions(false); setDoctorSuggestions([]); }
        return;
    }
    ...
    if (!isPharmacyField && !isDoctor && !isNurseWithOverride) return;

    let value = field === "timing" ? e : (e && e.target ? e.target.value : e);

    if (field === "qty" && (isDoctor || isNurseWithOverride)) {
        setQtyManuallyEntered((prev) => ({ ...prev, [type]: { ...prev[type], [index]: true } }));
        setShowQtySuggestions((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));
    }
    ...
    if (isDoctor || isNurseWithOverride) {
      setTimeout(() => {
        const currentStateMap = { tablets, injections, syrups, drops, creams, respules, lotions, fluids, powder, sutureProcedureItems, dressingItems, others };
        const currentItem = currentStateMap[type]?.[index];
        if (!currentItem) return;

        const medicineFormValue = medicineForms[type];
        const { chemicalName, brandName, doseVolume, expiryDate } = currentItem;
        ...
        if (standardSuggestionTypes.includes(type)) {
          if (field === "chemicalName") {
            ...
            fetchBrandSuggestions(chemicalName, medicineFormValue, type, index);
            if (chemicalName) fetchDoseSuggestions(value, chemicalName, medicineFormValue, type, index);
            setDoseManuallyEntered((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));
          } else if (field !== "doseVolume") {
            if (chemicalName && brandName) fetchDoseSuggestions(brandName, chemicalName, medicineFormValue, type, index);
          }

          if (["chemicalName", "brandName", "doseVolume"].includes(field)) {
            fetchExpiryDateSuggestions(chemicalName, brandName, doseVolume, type, index);
          }
  ...
  const handleSuggestionClick = (suggestion, type, index, field) => {
    ...
    setDoseManuallyEntered((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));
    setQtyManuallyEntered((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));
  ...
  const handleExpiryDateSuggestionClick = (suggestion, type, index) => {
    ...
    const updatedItem = getCurrentItemState(type, index);
    if (updatedItem) fetchQtySuggestions(updatedItem.chemicalName, updatedItem.brandName, suggestion, type, index);
    setShowExpiryDateSuggestions((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } }));
  };
  ...
  const renderDoseVolumeSuggestions = (type, index) => {
    ...
    if ((isDoctor || isNurseWithOverride) && show && suggestions.length > 0 && !doseManuallyEntered?.[type]?.[index]) {
      return (
        <div className="absolute z-20 bg-white border border-gray-300 rounded-md shadow-lg mt-1 w-full max-h-48 overflow-y-auto">
          {suggestions.map((suggestion, i) => (
  ...
  const renderQtySuggestions = (type, index) => {
      ...
      if ((isDoctor || isNurseWithOverride) && show && suggestions.length > 0 && !qtyManuallyEntered?.[type]?.[index]) {
          return (
              <div className="absolute z-20 bg-white border border-gray-300 rounded-md shadow-lg mt-1 w-full max-h-48 overflow-y-auto">
                  {suggestions.map((suggestion, i) => (
                      <div key={`${type}-${index}-qty-sugg-${i}`} className="px-4 py-2 hover:bg-blue-100 cursor-pointer text-sm"
                          onMouseDown={(e) => { e.preventDefault(); handleQtySuggestionClick(suggestion, type, index); }}>
                          {suggestion}
                      </div>
                  ))}
              </div>
  ...
  const renderExpiryDateSuggestions = (type, index) => {
    ...
    if ((isDoctor || isNurseWithOverride) && show && suggestions.length > 0) {
      return (
        <div className="absolute z-20 bg-white border border-gray-300 rounded-md shadow-lg mt-1 w-full max-h-48 overflow-y-auto">
          {suggestions.map((suggestion, i) => (
            <div key={`${type}-${index}-expiry-sugg-${i}`} className="px-4 py-2 hover:bg-blue-100 cursor-pointer text-sm"
              onMouseDown={(e) => { e.preventDefault(); handleExpiryDateSuggestionClick(suggestion, type, index); }}>
              {suggestion}
            </div>
          ))}
        </div>
  ...
  const renderInputFields = (type, items, index) => {
    ...
    const isFieldDisabledForCurrentUser = !isDoctor && !isNurseWithOverride;
    const isPharmacyFieldDisabledForCurrentUser = !isPharmacy;
    const fieldDisabledClass = isFieldDisabledForCurrentUser ? disabledClass : "";
    const pharmacyFieldDisabledClass = isPharmacyFieldDisabledForCurrentUser ? disabledClass : "";

    const reactSelectStyles = {
      control: (provided, state) => ({ ...provided, minHeight: "38px", height: "auto", borderColor: state.isFocused ? "#3b82f6" : "#d1d5db", boxShadow: state.isFocused ? "0 0 0 1px #3b82f6" : provided.boxShadow, borderRadius: "0.375rem", fontSize: "0.875rem", backgroundColor: isFieldDisabledForCurrentUser ? "#f3f4f6" : "white", opacity: isFieldDisabledForCurrentUser ? 0.7 : 1, cursor: isFieldDisabledForCurrentUser ? "not-allowed" : "default", "&:hover": { borderColor: state.isFocused ? "#3b82f6" : "#9ca3af" }, }),
      valueContainer: (provided) => ({ ...provided, padding: "1px 6px", alignItems: "center" }),
      input: (provided) => ({ ...provided, margin: "0px", padding: "0px", alignSelf: "stretch" }),
      indicatorSeparator: () => ({ display: "none" }),
      indicatorsContainer: (provided) => ({ ...provided, alignSelf: "stretch" }),
      menu: (provided) => ({ ...provided, fontSize: "0.875rem", zIndex: 30 }),
      multiValue: (provided) => ({ ...provided, backgroundColor: "#e0e7ff", margin: "2px", alignItems: "center" }),
      multiValueLabel: (provided) => ({ ...provided, padding: "2px", paddingLeft: "4px", whiteSpace: "normal" }),
      multiValueRemove: (provided) => ({ ...provided, "&:hover": { backgroundColor: "#be123c", color: "white" }, alignSelf: "center" }),
      ...
      return (
        <div className="relative">
          <input type="text" placeholder="Dose/Vol" value={item.doseVolume || ""}
            ...
            onFocus={() => { if (item.brandName && item.chemicalName && (isDoctor || isNurseWithOverride)) { fetchDoseSuggestions(item.brandName, item.chemicalName, medicineForms[type], type, index); } }}
    ...
    const renderQtyInput = () => (
        <div className="relative">
            <input type="text" placeholder="Qty" value={item.qty || ""}
                ...
                onFocus={() => { if (item.chemicalName && item.brandName && item.expiryDate && (isDoctor || isNurseWithOverride)) { fetchQtySuggestions(item.chemicalName, item.brandName, item.expiryDate, type, index); } }}
                onBlur={() => setTimeout(() => setShowQtySuggestions((prev) => ({ ...prev, [type]: { ...prev[type], [index]: false } })), 200)}
                className={`${inputBaseClass} ${fieldDisabledClass}`} disabled={isFieldDisabledForCurrentUser} autoComplete="off" />
            {renderQtySuggestions(type, index)}
        </div>
    );
    const renderServingInput = () => {
        const showField = ["tablets", "syrups", "drops", "lotions"].includes(type); if (!showField) return null;
        let placeholder = "Serving"; if (type === "tablets") placeholder = "(eg. 200mg)"; else if (type === "syrups") placeholder = "(e.g., 5ml)"; else if (type === "drops") placeholder = "(e.g., 1 drop)"; else if (type === "lotions") placeholder = "Application";
        return <input type="text" placeholder={placeholder} value={item.serving || ""} onChange={(e) => handleInputChange(e, type, index, "serving")} className={`${inputBaseClass} ${fieldDisabledClass}`} disabled={isFieldDisabledForCurrentUser} />;
    };
    const renderTimingSelect = () => {
      const showField = ["tablets", "syrups", "drops", "creams", "powder"].includes(type); if (!showField) return null;
      return <Select name="timing" isMulti options={timingOptions} value={item.timing || []} onChange={(selectedOptions) => handleInputChange(selectedOptions, type, index, "timing")} className={`${selectBaseClass} ${fieldDisabledClass}`} styles={reactSelectStyles} isDisabled={isFieldDisabledForCurrentUser} placeholder="Timing" closeMenuOnSelect={false} isClearable={false} />;
    };
    const renderFoodSelect = () => {
      const showField = ["tablets", "syrups", "drops", "creams"].includes(type); if (!showField) return null;
      return (
        <div className="relative">
          <select value={item.food || ""} onChange={(e) => handleInputChange(e, type, index, "food")} className={`${nativeSelectBaseClass} ${fieldDisabledClass}`} disabled={isFieldDisabledForCurrentUser}>
    ...
    const renderDaysInput = () => {
      const showField = ["tablets", "syrups", "drops", "creams", "respules", "lotions", "powder"].includes(type); if (!showField) return null;
      return <input type="text" placeholder="Days" value={item.days || ""} onChange={(e) => handleInputChange(e, type, index, "days")} className={`${inputBaseClass} ${fieldDisabledClass}`} disabled={isFieldDisabledForCurrentUser} />;
    };
    ...
    const renderPharmacyFields = () => (
      <>
        <textarea placeholder="Issued In" value={item.issuedIn || ""} onChange={(e) => handleInputChange(e, type, index, "issuedIn")} className={`${textareaBaseClass} ${pharmacyFieldDisabledClass} ${isPharmacy ? pharmacyEnabledClass : ""}`} disabled={isPharmacyFieldDisabledForCurrentUser} rows="1" title="Pharmacy Use Only: Issued In Details" />
        <textarea placeholder="Issued Out" value={item.issuedOut || ""} onChange={(e) => handleInputChange(e, type, index, "issuedOut")} className={`${textareaBaseClass} ${pharmacyFieldDisabledClass} ${isPharmacy ? pharmacyEnabledClass : ""}`} disabled={isPharmacyFieldDisabledForCurrentUser} rows="1" title="Pharmacy Use Only: Issued Out Details" />
        <textarea placeholder="Presc. Out" value={item.prescriptionOut || ""} onChange={(e) => handleInputChange(e, type, index, "prescriptionOut")} className={`${textareaBaseClass} ${pharmacyFieldDisabledClass} ${isPharmacy ? pharmacyEnabledClass : ""}`} disabled={isPharmacyFieldDisabledForCurrentUser} rows="1" title="Pharmacy Use Only: Prescription Out Details" />
      </>
    );

    switch (type) {
      case "tablets": case "syrups": case "drops":
        return <>{renderChemicalInput()}{renderBrandInput()}{renderDoseInput()}{renderExpiryDateInput()}{renderQtyInput()}{renderServingInput()}{renderTimingSelect()}{renderFoodSelect()}{renderDaysInput()}{renderPharmacyFields()}</>;
      case "creams": case "powder":
        return <>{renderChemicalInput()}{renderBrandInput()}{renderDoseInput()}{renderExpiryDateInput()}{renderQtyInput()}{renderTimingSelect()}{type === 'creams' && renderFoodSelect()}{renderDaysInput()}{renderPharmacyFields()}</>;
      case "injections": case "fluids": case "others":
        return <>{renderChemicalInput()}{renderBrandInput()}{renderDoseInput()}{renderExpiryDateInput()}{renderQtyInput()}{renderPharmacyFields()}</>;
      case "respules":
        return <>{renderChemicalInput()}{renderBrandInput()}{renderDoseInput()}{renderExpiryDateInput()}{renderQtyInput()}{renderDaysInput()}{renderPharmacyFields()}</>;
      case "lotions":
        return <>{renderChemicalInput()}{renderBrandInput()}{renderDoseInput()}{renderExpiryDateInput()}{renderQtyInput()}{renderServingInput()}{renderDaysInput()}{renderPharmacyFields()}</>;
      case "sutureProcedureItems": case "dressingItems":
        return <><div className="relative"><input type="text" placeholder="Item Name" value={item.chemicalName || ""} onChange={(e) => handleInputChange(e, type, index, "chemicalName")} className={`${inputBaseClass} ${fieldDisabledClass}`} disabled={isFieldDisabledForCurrentUser} autoComplete="off" /></div>{renderExpiryDateInput()}{renderQtyInput()}{renderPharmacyFields()}</>;
      default: return null;
    }
  };
  ...
  const handleSubmit = async (mrdNumber) => {
    try {
      ...
      const issuedItemsForStock = filterEmptyRows(allItemsForStock).filter(item => item.issuedIn && parseInt(item.issuedIn) > 0);

      if (response.data) {
        alert("Prescription saved successfully!");
        if (onPrescriptionUpdate) onPrescriptionUpdate(issuedItemsForStock);
      }
    } catch (error) { console.error("Error saving prescription:", error); setError(error.response?.data?.error || "Error saving prescription"); alert(error.response?.data?.error || "Error saving prescription"); }
  };

  const handleGeneratePrescription = () => {
    if (!emp_no || !data?.[0]?.name) { alert("Cannot generate: Employee details missing."); return; }
    const allSections = { Tablets: tablets, Injections: injections, Syrups: syrups, Drops: drops, "Creams & Ointments": creams, Respules: respules, Lotions: lotions, Fluids: fluids, Powders: powder, "Suture & Procedure Items": sutureProcedureItems, "Dressing Items": dressingItems, Others: others };
    ...
    if (Object.keys(filteredSections).length === 0 && !nurseNotes.trim() && (!isNurseWithOverride || !consultedDoctor.trim())) {
        alert("Cannot generate an empty prescription. Please add medication, notes, or consultation details."); return;
    }

    const doc = new jsPDF("p", "mm", "a4");
    const pageWidth = doc.internal.pageSize.getWidth(); const margin = 15; const lineHeight = 7; const sectionSpacing = 10; let y = margin;
    ...
    const addText = (text, x, currentY, options = {}) => {
      ...
      const textHeight = lines.length * lineHeight * 0.8 + (isTitle ? lineHeight * 0.5 : 0);
      if (currentY + textHeight > doc.internal.pageSize.getHeight() - margin) {
        doc.addPage(); currentY = margin;
        doc.setFontSize(8); doc.setFont(undefined, "italic"); doc.text(`Prescription for ${data[0].name} (${emp_no}) - Page ${doc.internal.getNumberOfPages()}`, pageWidth - margin, margin / 2, { align: "right" }); doc.setFont(undefined, "normal"); currentY += lineHeight / 2;
      }
      doc.setFontSize(fontSize); doc.setFont(undefined, isBold || isTitle ? "bold" : "normal");
      let textX = x; let textAlign = "left"; if (isCentered) { textX = pageWidth / 2; textAlign = "center"; }
      const textOptions = { align: textAlign };
      if (maxWidth) { doc.text(lines, textX, currentY, textOptions); currentY += lines.length * lineHeight * 0.8; }
      else { doc.text(textString, textX, currentY, textOptions); currentY += isTitle ? lineHeight * 1.5 : lineHeight; }
      return currentY;
    };

    y = addText("PRESCRIPTION", margin, y, { isTitle: true, fontSize: 16, isCentered: true }); y += lineHeight / 2;
    const patientBoxStartY = y; doc.setFontSize(11); doc.setFont(undefined, "bold"); y = addText("Patient Information", margin, y); doc.setFontSize(10); doc.setFont(undefined, "normal");
    y = addText(`Employee No: ${emp_no}`, margin + 5, y); y = addText(`Patient Name: ${data[0].name}`, margin + 5, y); y = addText(`Date: ${new Date().toLocaleDateString("en-GB")}`, margin + 5, y);
    const patientBoxEndY = y; doc.setDrawColor(150, 150, 150); doc.rect(margin - 2, patientBoxStartY - lineHeight * 0.8, pageWidth - (margin - 2) * 2, patientBoxEndY - patientBoxStartY + lineHeight * 0.5); y += sectionSpacing / 1.5;
    if(isNurseWithOverride && consultedDoctor.trim()){
        const consultBoxStartY = y; doc.setFontSize(11); doc.setFont(undefined, "bold"); y = addText("Consultation Details", margin, y); doc.setFontSize(10); doc.setFont(undefined, "normal");
        y = addText(`Consulted Doctor: ${consultedDoctor}`, margin + 5, y);
        const consultBoxEndY = y; doc.setDrawColor(150, 150, 150); doc.rect(margin - 2, consultBoxStartY - lineHeight * 0.8, pageWidth - (margin - 2) * 2, consultBoxEndY - consultBoxStartY + lineHeight * 0.5); y += sectionSpacing / 1.5;
    }
    doc.setFontSize(24); doc.setFont(undefined, "bold"); y = addText("Rx", margin, y); y += lineHeight / 2;

    const formatTimingForPDF = (timing) => Array.isArray(timing) ? timing.map(t => t.label || t.value).join(", ") || "N/A" : "N/A";
    ...
    for (const [title, items] of Object.entries(filteredSections)) {
      ...
      items.forEach((item, index) => {
        ...
        if (y + estimatedItemHeight > doc.internal.pageSize.getHeight() - margin) {
          doc.addPage(); y = margin;
          doc.setFontSize(8); doc.setFont(undefined, "italic"); doc.text(`Prescription for ${data[0].name} (${emp_no}) - Page ${doc.internal.getNumberOfPages()}`, pageWidth - margin, margin / 2, { align: "right" }); doc.setFont(undefined, "normal"); y += lineHeight / 2;
          y = addText(title + " (cont.)", margin, y, { isBold: true, fontSize: 12 });
        }
        let mainLine = `${index + 1}. ${item.chemicalName || "N/A"}`; if (item.brandName) mainLine += ` (${item.brandName})`; if (item.doseVolume) mainLine += ` - ${item.doseVolume}`;
        y = addText(mainLine, margin + 5, y);
        let qtyServingLineParts = []; if (item.qty) qtyServingLineParts.push(`Qty: ${item.qty}`); if (item.serving) qtyServingLineParts.push(`Serving: ${item.serving}`);
        if (qtyServingLineParts.length > 0) y = addText(`   ${qtyServingLineParts.join(" | ")}`, margin + 10, y, { fontSize: 9 });
        let sigLineParts = []; const formattedTiming = formatTimingForPDF(item.timing); if (formattedTiming && formattedTiming !== "N/A") sigLineParts.push(`Timing: ${formattedTiming}`); if (item.food) sigLineParts.push(`Food: ${item.food}`); if (item.days) sigLineParts.push(`Days: ${item.days}`);
        if (sigLineParts.length > 0) y = addText(`   Sig: ${sigLineParts.join(" | ")}`, margin + 10, y, { fontSize: 9 });
        if (item.expiryDate) y = addText(`   Expiry: ${item.expiryDate}`, margin + 10, y, { fontSize: 9 });
        y += lineHeight * 0.4;
      });
      y += sectionSpacing / 1.5;
    }
    doc.setDrawColor(150, 150, 150); doc.line(margin, y - lineHeight / 2, pageWidth - margin, y - lineHeight / 2); y += lineHeight / 2;
    if (nurseNotes && nurseNotes.trim()) {
      y = addText("Nurse Notes:", margin, y, { isBold: true, fontSize: 11 });
      y = addText(nurseNotes, margin, y, { fontSize: 10, maxWidth: pageWidth - margin * 2 }); y += sectionSpacing / 2;
    }
    const footerStartY = y; const footerHeightEstimate = lineHeight * 4;
    if (footerStartY + footerHeightEstimate > doc.internal.pageSize.getHeight() - margin) {
        doc.addPage(); y = margin;
        doc.setFontSize(8); doc.setFont(undefined, "italic"); doc.text(`Prescription for ${data[0].name} (${emp_no}) - Page ${doc.internal.getNumberOfPages()}`, pageWidth - margin, margin / 2, { align: "right" }); doc.setFont(undefined, "normal"); y += lineHeight;
    } else { y = footerStartY; }
    const signatureX = pageWidth / 2 + 10; const signatureY = y + lineHeight * 1.5;
    doc.setDrawColor(0, 0, 0); doc.line(signatureX, signatureY, pageWidth - margin, signatureY);
  ...
  const gridTemplateColumns = {
    tablets: "repeat(4, minmax(140px, 1.5fr)) repeat(2, minmax(80px, 0.8fr)) repeat(3, minmax(100px, 1fr)) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    syrups: "repeat(4, minmax(140px, 1.5fr)) repeat(2, minmax(80px, 0.8fr)) repeat(3, minmax(100px, 1fr)) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    drops: "repeat(4, minmax(140px, 1.5fr)) repeat(2, minmax(80px, 0.8fr)) repeat(3, minmax(100px, 1fr)) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    creams: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) repeat(3, minmax(100px, 1fr)) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    powder: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) repeat(2, minmax(100px, 1fr)) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    injections: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    fluids: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    others: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    respules: "repeat(4, minmax(140px, 1.5fr)) minmax(80px, 0.8fr) minmax(100px, 1fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    lotions: "repeat(4, minmax(140px, 1.5fr)) repeat(2, minmax(80px, 0.8fr)) minmax(100px, 1fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    sutureProcedureItems: "minmax(150px, 2fr) minmax(80px, 1fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
    dressingItems: "minmax(150px, 2fr) minmax(80px, 1fr) minmax(100px, 1.2fr) repeat(3, minmax(120px, 1fr)) minmax(40px, auto)",
  };
  ...
  const renderSection = (type, title, color, children, addItemButtonText) => {
    ...
    const getHeaders = () => {
      const baseHeaders = [{ title: "Chemical Name", gridSpan: "span 1", tooltip: "Generic name" }];
      const brandDoseHeaders = [{ title: "Brand Name", gridSpan: "span 1", tooltip: "Brand name" }, { title: "Dose/Vol", gridSpan: "span 1", tooltip: "Dosage" }];
      const expiryHeader = { title: "Expiry Date", gridSpan: "span 1", tooltip: "Expiration date" };
      const qtyHeader = { title: "Qty", gridSpan: "span 1", tooltip: "Quantity" };
      const servingHeader = { title: "Serving", gridSpan: "span 1", tooltip: "Dosage per administration" };
      const timingHeader = { title: "Timing", gridSpan: "span 1", tooltip: "When to take" };
      const foodHeader = { title: "Food", gridSpan: "span 1", tooltip: "Relation to food" };
      const daysHeader = { title: "Days", gridSpan: "span 1", tooltip: "Duration" };
      const pharmacyHeaders = [{ title: "Issued In", gridSpan: "span 1", tooltip: "Pharmacy: Arrival" }, { title: "Issued Out", gridSpan: "span 1", tooltip: "Pharmacy: Dispensing" }, { title: "Presc. Out", gridSpan: "span 1", tooltip: "Pharmacy: Status" }];
      const actionHeader = { title: "Action", gridSpan: "span 1", tooltip: "Remove row" };
      let headers = [];
      switch (type) {
        case "tablets": case "syrups": case "drops": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, servingHeader, timingHeader, foodHeader, daysHeader, ...pharmacyHeaders, actionHeader]; break;
        case "creams": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, timingHeader, foodHeader, daysHeader, ...pharmacyHeaders, actionHeader]; break;
        case "powder": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, timingHeader, daysHeader, ...pharmacyHeaders, actionHeader]; break;
        case "injections": case "fluids": case "others": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, ...pharmacyHeaders, actionHeader]; break;
        case "respules": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, daysHeader, ...pharmacyHeaders, actionHeader]; break;
        case "lotions": headers = [...baseHeaders, ...brandDoseHeaders, expiryHeader, qtyHeader, servingHeader, daysHeader, ...pharmacyHeaders, actionHeader]; break;
        case "sutureProcedureItems": case "dressingItems": headers = [{ title: "Item Name", gridSpan: "span 1", tooltip: "Item name" }, expiryHeader, qtyHeader, ...pharmacyHeaders, actionHeader]; break;
        default: return null;
      }
      return headers.filter(h => h).map((h, i) => <div key={`${type}-header-${i}`} className="font-medium text-xs text-gray-600 truncate uppercase tracking-wider" title={h.tooltip} style={{ gridColumn: h.gridSpan }}>{h.title}</div>);
    };

    return (
      <section className="border border-gray-200 rounded-lg shadow-sm mb-6 overflow-hidden bg-white">
        <h2 className={`text-lg font-semibold p-3 bg-${headerColor}-50 border-b border-gray-200 text-${headerColor}-700 flex justify-between items-center cursor-pointer hover:bg-${headerColor}-100 transition-colors duration-150`} onClick={() => toggleSection(type)} aria-expanded={isExpanded} aria-controls={`section-content-${type}`}>
          {title} <span className={`text-sm text-gray-500 transition-transform duration-200 transform ${isExpanded ? "rotate-180" : "rotate-0"}`}>▼</span>
        </h2>
        {isExpanded && (
          <div id={`section-content-${type}`} className="p-4">
            <div className="overflow-x-auto pb-2">
              <div className={`grid gap-x-4 items-center mb-3 pb-2 border-b border-gray-200 px-1`} style={{ gridTemplateColumns: gridTemplateColumns[type] }}>{getHeaders()}</div>
              <div>{children}</div>
            </div>
            <div className="mt-4">
              <ActionButton onClick={() => addRow(type)} disabled={!isDoctor && !isNurseWithOverride} color={buttonColor} title={ (isDoctor || isNurseWithOverride) ? addItemButtonText : "Only doctors or authorized nurses can add items"}>
                + {addItemButtonText}
              </ActionButton>
            </div>
          </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 md:p-6 space-y-6">
      <h1 className="text-xl md:text-2xl font-bold text-gray-800 border-b pb-2 mb-6">
        ...
        {existingPrescription?.id && <span className="text-sm text-gray-500 font-normal ml-2">(ID: {existingPrescription.id})</span>}
      </h1>

      {isNurseWithOverride && (
        <section className="border border-gray-200 rounded-lg shadow-sm mb-6 overflow-hidden bg-white">
          <h2 className="text-lg font-semibold p-3 bg-yellow-50 border-b border-gray-200 text-yellow-700">Consultation Details</h2>
          <div className="p-4">
            <label htmlFor="consultedDoctor" className="block text-sm font-medium text-gray-700 mb-1">Consulted Doctor Name</label>
            <div className="relative">
              <input type="text" id="consultedDoctor" name="consultedDoctor" placeholder="Enter doctor's name" value={consultedDoctor}
                ...
                autoComplete="off" disabled={!isNurseWithOverride} />
              {showDoctorSuggestions && doctorSuggestions.length > 0 && (
                <div className="absolute z-30 bg-white border border-gray-300 rounded-md shadow-lg mt-1 w-full max-h-48 overflow-y-auto">
                  {doctorSuggestions.map((doctor, i) => (
                    <div key={`doc-sugg-${i}`} className="px-4 py-2 hover:bg-blue-100 cursor-pointer text-sm"
                      onMouseDown={(e) => { e.preventDefault(); handleDoctorSuggestionClick(doctor.name || doctor); }}>
                      {doctor.name || doctor}
                    </div>
                  ))}
                </div>
      ...
      {sectionOrder.map((type) => {
        ...
        const children = items.map((_, index) => (
          <div key={`${type}-${index}`} className={`grid gap-x-4 mb-3 items-start px-1`} style={{ gridTemplateColumns: gridTemplateColumns[type] }}>
            {renderInputFields(type, items, index)}
            <div className="flex justify-center items-center h-full pt-1">
              {(isDoctor || isNurseWithOverride) ? <RemoveButton onClick={removeRow} type={type} index={index} disabled={!isDoctor && !isNurseWithOverride} /> : <div className="w-8 h-8"></div>}
            </div>
          </div>
        ));
